...
def add_missing_protoc_platforms(ns: str, root: ET.Element):
    platforms = [
        'linux-aarch_64',
        'linux-ppcle_64',
        'linux-s390_64',
    ...
    add_missing_components(
        ...
        'protoc',
        lambda info: ArtifactAssets(
            repo=MAVEN_CENTRAL_REPO,
            files=[f'{info.name}-{info.version}-{p}.exe' for p in platforms],
        ),
    )
...
def patch_xml(path: str | os.PathLike[str]):
    ...
    add_missing_protoc_platforms(ns, root)
